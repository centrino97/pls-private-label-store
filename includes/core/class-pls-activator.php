...
final class PLS_Activator {
    ...
    private static function maybe_run_migrations() {
        ...
        // Run v0.10.0 migration if upgrading from earlier version
        if ( version_compare( $stored_version, '0.10.0', '<' ) ) {
        ...
        // Run v1.3.0 migration if upgrading from earlier version
        if ( version_compare( $stored_version, '1.3.0', '<' ) ) {
            require_once PLS_PLS_DIR . 'includes/core/class-pls-migration-v130.php';
            PLS_Migration_v130::run();
        }
        
        // Run v2.3.0 migration if upgrading from earlier version
