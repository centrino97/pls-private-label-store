    ...
    $.post(plsOffers.ajaxUrl, { action:'pls_get_offers', nonce: plsOffers.nonce }, function(res){
      ...
      offers.forEach(function(o){
        ...
        var $btn = $('<button/>').addClass('pls-offer__btn').text('Apply').on('click', function(){
          $.post(plsOffers.ajaxUrl, { action:'pls_apply_offer', nonce: plsOffers.nonce, offer_id:o.id }, function(r){
      ...
      $.ajax({
        ...
        success: function(response) {
          if (response.success) {
            ...
            // Trigger cart update event (safely handle optional properties)
            const fragments = (response.data && response.data.fragments) ? response.data.fragments : {};
            const cartHash = (response.data && response.data.cart_hash) ? response.data.cart_hash : '';
